...
const ConfigSchema = z.object({
  ...
  PM_ALLOWED_COMMANDS: z.string().transform(val => val.split(',')).default('pwd,/bin,/usr/bin,/usr/local/bin,/opt/homebrew/bin'),
  PM_ALLOWED_TOOL_NAMES: z.string().transform(val => val.split(',')).default('node,npm,pnpm,yarn,npx,tsc,tsx,ts-node,next,vite,vitest,git,ls,bash,sh,curl,bun'),
