...
- JSON‑RPC error semantics
  - Invalid params: throw to let SDK produce JSON‑RPC -32602; use `isError` only for domain‑level failures if needed.
- Size limits
  - Impose max error message/stack size before insert to prevent oversized rows.
- Tools registry & MCP shape
  - Register error tools via central registry; `tools/list` returns JSON Schema.
- TDD additions
  - Tests: categorization patterns; resolution updates; summary & trend accuracy under time windows; similar error matching; schema validation.

### Revised Error Tools Example
```typescript
// src/tools/errors.ts
import { z } from 'zod';
...
const GetErrorsSchema = z.object({ processId: z.string().optional(), limit: z.number().min(1).max(200).default(50) });
const GetLatestErrorsSchema = z.object({ limit: z.number().min(1).max(200).default(20) });
const ResolveErrorSchema = z.object({ errorId: z.number().min(1) });

export function registerErrorTools(errors: ErrorManager, logger: winston.Logger) {
  registerTool({
    name: 'get_errors', description: 'Retrieve error history', schema: GetErrorsSchema,
    handler: async (args) => {
      const { processId, limit } = args as any;
      const list = await errors.getErrors({ processId, limit });
      return [{ type: 'text', text: `Errors: ${list.length}` }];
    },
  });

  registerTool({
    name: 'get_latest_errors', description: 'Recent errors across all processes', schema: GetLatestErrorsSchema,
    handler: async ({ limit }: any) => {
      const list = await errors.getLatestErrors(limit, true);
      return [{ type: 'text', text: `Latest errors: ${list.length}` }];
    },
  });

  registerTool({
    name: 'mark_error_resolved', description: 'Mark an error as resolved', schema: ResolveErrorSchema,
    handler: async ({ errorId }: any) => {
      await errors.markResolved(errorId);
      return [{ type: 'text', text: `Resolved error ${errorId}` }];
    },
  });
}
```
